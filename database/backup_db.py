...
from logger import log_event_async
    ...
    if len(backup_files) > max_backups:
        ...
        for file in old_backups:
            try:
                ...
                log_event_async("backup_cleanup", "system", f"Deleted old {backup_type} backup: {file}")
            except Exception as e:
                print(f"Error deleting {file}: {e}")
                log_event_async("backup_cleanup_error", "system", f"Failed to delete {file}: {e}")
...
def main():
    ...
    if daily_backup_needed:
        ...
        if create_backup(daily_backup_path):
            ...
            log_event_async("backup_success", "system", f"Daily backup created: {daily_backup_path}")
            
            # Cleanup old daily backups
            cleanup_old_backups(DAILY_BACKUP_DIR, MAX_DAILY_BACKUPS, "daily")
        else:
            print("❌ Daily backup failed!")
            log_event_async("backup_failed", "system", "Daily backup failed")
