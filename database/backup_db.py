...
def cleanup_old_backups(backup_dir, max_backups, backup_type):
    ...
    await log_event("backup_cleanup", "system", f"Deleted old {backup_type} backup: {file}")
            except Exception as e:
                print(f"Error deleting {file}: {e}")
    await log_event("backup_cleanup_error", "system", f"Failed to delete {file}: {e}")
...
def main():
    ...
    await log_event("backup_success", "system", f"Daily backup created: {daily_backup_path}")
            
            # Cleanup old daily backups
            cleanup_old_backups(DAILY_BACKUP_DIR, MAX_DAILY_BACKUPS, "daily")
        else:
            print("❌ Daily backup failed!")
    await log_event("backup_failed", "system", "Daily backup failed")
