...
from logger import log_event_async as log_event, log_error, log_critical
...
async def run_maintenance():
    ...
    try:
        ...
        try:
            ...
            print(f"❌ {error_msg}")
            log_error("maintenance_auto_deletion_failed", "system", error_msg)
        ...
        try:
            ...
            logger.error(error_msg)
            print(f"❌ {error_msg}")
            log_error("maintenance_backup_failed", "system", error_msg)
        ...
        try:
            ...
            logger.error(error_msg)
            print(f"❌ {error_msg}")
            log_error("maintenance_log_cleanup_failed", "system", error_msg)
        ...
        log_event("maintenance_completed", "system", 
                 f"Maintenance completed with status: {maintenance_results['overall_status']}", 
        ...
        print(f"❌ {error_msg}")
        log_critical("maintenance_script_failed", "system", error_msg)
        sys.exit(1)
