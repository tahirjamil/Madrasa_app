...
class BaseAuthRequest(BaseModel):
    ...
    @validator('fullname', 'phone', 'device_id', 'device_model', 'device_os', 'device_brand')
    ...
    @validator('*')
    def check_sql_injection(cls, v, field):
        if isinstance(v, str) and security_manager.detect_sql_injection(v):
            raise ValueError(f"Invalid input detected in {field.name}")
        return v
