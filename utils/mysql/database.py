    ...
    try:
        ...
        if config.OTEL_ENABLED:
            ...
            try:
                ...
                if config.OTEL_STRICT:
                    raise RuntimeError(f"OTEL SQLAlchemy instrumentation failed: {e}")
                else:
                    log.warning(
                        action="otel_instrumentation_warning",
                        message=f"OTEL instrumentation failed, continuing without it: {type(e).__name__}"
                    )
